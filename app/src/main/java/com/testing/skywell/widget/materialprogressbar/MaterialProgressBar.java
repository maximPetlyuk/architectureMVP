...
import com.testing.skywell.widget.materialprogressbar.internal.DrawableCompat;
...
public class MaterialProgressBar extends ProgressBar {
	...
	private void init(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
		...
		boolean showTrack = a.getBoolean(R.styleable.MaterialProgressBar_mpb_showTrack, mProgressStyle == PROGRESS_STYLE_HORIZONTAL);
		if (a.hasValue(R.styleable.MaterialProgressBar_android_tint))
		{
			mProgressTint.mTintList = a.getColorStateList(R.styleable.MaterialProgressBar_android_tint);
			mProgressTint.mHasTintList = true;
		}
		if (a.hasValue(R.styleable.MaterialProgressBar_mpb_tintMode))
		{
			mProgressTint.mTintMode = DrawableCompat.parseTintMode(a.getInt(R.styleable.MaterialProgressBar_mpb_tintMode, -1), null);
	...
	public void setProgressColor(int _color) {
		int[][] states = new int[][] { new int[] { android.R.attr.state_enabled }, // enabled
				new int[] { -android.R.attr.state_enabled }, // disabled
				new int[] { -android.R.attr.state_checked }, // unchecked
				new int[] { android.R.attr.state_pressed } // pressed
		};
		int[] colors = new int[] { _color, _color, _color, _color};
		ColorStateList myList = new ColorStateList(states, colors);
		Drawable draw = getIndeterminateDrawable();
		TintInfo info = new TintInfo();
		info.mHasTintList = true;
		info.mTintList = myList;
		applyTintForDrawable(draw, info);
	}
	 ...
	 *             If the progress drawable does not implement {@link IntrinsicPaddingDrawable}.
	 */
	 ...
	 * 
	 * @param useIntrinsicPadding
	 *            Whether the progress drawable should use its intrinsic padding.
	 * @throws IllegalStateException
	 *             If the progress drawable does not implement {@link IntrinsicPaddingDrawable}.
	 */
	 ...
	 * 
	 * @return Whether the progress drawable is showing a track, or {@code false} if the drawable does not implement {@link ShowTrackDrawable}.
	 */
	 ...
	 * 
	 * @param showTrack
	 *            Whether track should be shown. When {@code false}, does nothing if the progress drawable does not implement {@link ShowTrackDrawable}. When {@code true}, throws {@link IllegalStateException}.
	 * @throws IllegalStateException
	 *             If {@code showTrack} is {@code true} but the progress drawable does not implement {@link ShowTrackDrawable}.
	 */
	...
	/**
	 * Applies a tint to the progress drawable. Does not modify the current tint mode, which is {@link PorterDuff.Mode#SRC_IN} by default.
	 * <p>
	 * Subsequent calls to {@link #setBackground(Drawable)} will automatically mutate the drawable and apply the specified tint and tint mode using {@link Drawable#setTintList(ColorStateList)}.
	 * 
	 * @param tint
	...
	/**
	 * Specifies the blending mode used to apply the tint specified by {@link #setProgressTintList(ColorStateList)} to the progress drawable. The default mode is {@link PorterDuff.Mode#SRC_IN}.
	 * 
	 * @param tintMode
	...
	// compatibility.
	@SuppressLint("NewApi")
	public void applyTintForDrawable(Drawable drawable, TintInfo tint) {
