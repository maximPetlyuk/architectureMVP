...
/**
 * Helper for accessing features in {@link ObjectAnimator} introduced after API level 11 (for {@link android.animation.PropertyValuesHolder}) in a backward compatible fashion.
 */
public class ObjectAnimatorCompat {
	...
	/**
	 * Constructs and returns an ObjectAnimator that animates between color values. A single value implies that that value is the one being animated to. Two values imply starting and ending values. More than two values imply a starting value, values to
	 * animate through along the way, and an ending value (these values will be distributed evenly across the duration of the animation).
	 * 
	 * @param target
	 *            The object whose property is to be animated. This object should have a public method on it called {@code setName()}, where {@code name} is the value of the {@code propertyName} parameter.
	 * @param propertyName
	...
	/**
	 * Constructs and returns an ObjectAnimator that animates coordinates along a {@code Path} using two properties. A {@code Path} animation moves in two dimensions, animating coordinates {@code (x, y)} together to follow the line. In this variation, the
	 * coordinates are floats that are set to separate properties designated by {@code xPropertyName} and {@code yPropertyName}.
	 * 
	 * @param target
	 *            The object whose properties are to be animated. This object should have public methods on it called {@code setNameX()} and {@code setNameY}, where {@code nameX} and {@code nameY} are the value of the {@code xPropertyName} and
	 ...
	 * @param path
	 *            The {@code Path} to animate values along.
	...
	/**
	 * Constructs and returns an ObjectAnimator that animates coordinates along a {@code Path} using two properties. A {@code Path} animation moves in two dimensions, animating coordinates {@code (x, y)} together to follow the line. In this variation, the
	 * coordinates are floats that are set to separate properties, {@code xProperty} and {@code yProperty}.
	 * 
	 * @param target
	...
	/**
	 * Constructs and returns an ObjectAnimator that animates coordinates along a {@code Path} using two properties. A {@code Path} animation moves in two dimensions, animating coordinates {@code (x, y)} together to follow the line. In this variation, the
	 * coordinates are integers that are set to separate properties designated by {@code xPropertyName} and {@code yPropertyName}.
	 * 
	 * @param target
	 *            The object whose properties are to be animated. This object should have public methods on it called {@code setNameX()} and {@code setNameY}, where {@code nameX} and {@code nameY} are the value of {@code xPropertyName} and
	 *            {@code yPropertyName} parameters, respectively.
	 * @param xPropertyName
	 *            The name of the property for the x coordinate being animated.
	 * @param yPropertyName
	...
	/**
	 * Constructs and returns an ObjectAnimator that animates coordinates along a {@code Path} using two properties. A {@code Path} animation moves in two dimensions, animating coordinates {@code (x, y)} together to follow the line. In this variation, the
	 * coordinates are integers that are set to separate properties, {@code xProperty} and {@code yProperty}.
	 * 
	 * @param target
	 *            The object whose properties are to be animated.
	 * @param xProperty
